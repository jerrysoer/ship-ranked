...
const PLATFORM_QUERIES = [
  ...
  {
    endpoint: '/search/code',
    q: 'filename:openclaw.json pushed:>2024-01-01 stars:>3',
    signal: 'openclaw-json',
    hintPlatform: 'openclaw',
    extractRepo: (item) => item.repository,
  },
  {
    endpoint: '/search/repositories',
    q: 'topic:openclaw pushed:>2024-01-01 stars:>2',
    signal: 'topic-openclaw',
    hintPlatform: 'openclaw',
    extractRepo: (item) => item,
  },
  {
    endpoint: '/search/repositories',
    q: 'topic:openclaw-ai pushed:>2024-01-01 stars:>2',
    signal: 'topic-openclaw-ai',
    hintPlatform: 'openclaw',
    extractRepo: (item) => item,
  },
  // Codex CLI
  {
    endpoint: '/search/code',
    q: 'filename:AGENTS.md topic:codex-cli pushed:>2025-01-01 stars:>3',
    signal: 'agents-md-codex',
    hintPlatform: 'codex',
    extractRepo: (item) => item.repository,
  },
  {
    endpoint: '/search/repositories',
    q: 'topic:openai-codex pushed:>2025-01-01 stars:>2',
    signal: 'topic-codex-openai',
    hintPlatform: 'codex',
    extractRepo: (item) => item,
  },
  {
    endpoint: '/search/code',
    q: 'filename:codex.md pushed:>2025-01-01 stars:>3',
    signal: 'codex-md',
    hintPlatform: 'codex',
    extractRepo: (item) => item.repository,
  },
  ...
  {
    endpoint: '/search/code',
    q: 'filename:mcp.json pushed:>2024-06-01 stars:>3',
    signal: 'mcp-json',
    hintPlatform: 'mcp',
    extractRepo: (item) => item.repository,
  },
  {
    endpoint: '/search/repositories',
    q: 'topic:mcp-server pushed:>2024-06-01 stars:>3',
    signal: 'topic-mcp-server',
    hintPlatform: 'mcp',
    extractRepo: (item) => item,
  },
  {
    endpoint: '/search/repositories',
    q: 'topic:model-context-protocol pushed:>2024-06-01 stars:>3',
    signal: 'topic-mcp',
    hintPlatform: 'mcp',
    extractRepo: (item) => item,
  },
  {
    endpoint: '/search/code',
    q: '"@modelcontextprotocol/sdk" filename:package.json pushed:>2024-06-01 stars:>5',
    signal: 'mcp-sdk-npm',
    hintPlatform: 'mcp',
    extractRepo: (item) => item.repository,
  },
...
// In-memory cache for file tree fetches within a single pipeline run
const fileTreeCache = new Map()

async function fetchRootFileTree(fullName, token) {
  if (fileTreeCache.has(fullName)) return fileTreeCache.get(fullName)
  try {
    const res = await fetch(`${GH_API}/repos/${fullName}/contents`, {
      headers: {
        Authorization: `Bearer ${token}`,
        Accept: 'application/vnd.github+json',
        'X-GitHub-Api-Version': '2022-11-28',
    ...
    if (!res.ok) {
      fileTreeCache.set(fullName, null)
      return null
    }
    const data = await res.json()
    const tree = Array.isArray(data) ? data.map(f => ({ path: f.name, name: f.name })) : null
    fileTreeCache.set(fullName, tree)
    return tree
  } catch (e) {
    console.error(`File tree fetch failed for ${fullName}:`, e.message)
    fileTreeCache.set(fullName, null)
    return null
  }
}

async function fetchReadmeText(fullName, token) {
  try {
    ...
    console.error(`README text fetch failed for ${fullName}:`, e.message)
    return null
  }
}
...
function detectPlatform(repo, fileTree, readmeText) {
  ...
  // Priority 0: MCP Server — definitive markers
  if (files.includes('mcp.json')) return 'mcp'
  if (topics.includes('mcp-server') || topics.includes('model-context-protocol')) return 'mcp'
  if (readme.includes('@modelcontextprotocol/sdk') ||
      readme.includes('pip install mcp') ||
      (readme.includes('mcp server') && readme.includes('tool'))) return 'mcp'
  ...
  for (const project of unstarred || []) {
    try {
      ...
      if (res.ok || res.status === 204) {
        await supabasePatch('ranked_projects', project.id, { starred_by_shipranked: true })
        count++
      }
    } catch (e) {
...
export async function runPipeline() {
  ...
  // --- 2. Run GitHub searches sequentially (multi-platform) ---
  const repoMap = new Map()       // full_name -> repo data
  const signalMap = new Map()     // full_name -> signal string
  const hintPlatformMap = new Map() // full_name -> hinted platform from query
  ...
  for (const query of PLATFORM_QUERIES) {
    ...
    for (const item of items) {
      ...
      // Track hint platform from query (used for optimization)
      if (!hintPlatformMap.has(name)) {
        hintPlatformMap.set(name, query.hintPlatform)
      }
    }
  ...
  for (const [name, repo] of repoMap) {
    ...
    if (isValidProject(repo)) {
      filtered.push({ name, repo, signal: signalMap.get(name) })
    }
  }

  // --- 3b. Detect platform for each repo ---
  // Clear file tree cache for this run
  fileTreeCache.clear()
  let fileTreeFetches = 0
  const FILE_TREE_BUDGET = 150

  for (const entry of filtered) {
    const signal = entry.signal
    const repo = entry.repo

    // Optimization: definitive signals skip file tree fetch
    if (signal === 'claude-md') {
      entry.agentPlatform = 'claude-code'
      continue
    }
    if (signal === 'openclaw-json') {
      entry.agentPlatform = 'openclaw'
      continue
    }
    if (signal === 'mcp-json') {
      entry.agentPlatform = 'mcp'
      continue
    }
    // MCP topic signals are strong enough to skip file tree fetch
    if (signal === 'topic-mcp-server' || signal === 'topic-mcp' || signal === 'mcp-sdk-npm') {
      entry.agentPlatform = 'mcp'
      continue
    }

    // Fetch file tree for detection (within budget)
    let fileTree = null
    if (fileTreeFetches < FILE_TREE_BUDGET) {
      fileTree = await fetchRootFileTree(entry.name, ghToken)
      fileTreeFetches++
      await sleep(100) // rate limit courtesy
    }

    // Fetch README when needed for disambiguation:
    // - AGENTS.md present but no definitive agent marker
    // - MCP hint but no mcp.json (need README for heuristic check)
    const files = (fileTree || []).map(f => f.path.toLowerCase())
    const hintPlatform = hintPlatformMap.get(entry.name)
    let readmeText = null
    if ((files.includes('agents.md') && !files.includes('claude.md') && !files.includes('openclaw.json')) ||
        (hintPlatform === 'mcp' && !files.includes('mcp.json'))) {
      readmeText = await fetchReadmeText(entry.name, ghToken)
      await sleep(100)
    }

    entry.agentPlatform = detectPlatform(repo, fileTree, readmeText)
  }
  ...
  // --- 5. Sort by momentum and assign rank (two-pass: agent vs MCP) ---
  const agentEntries = filtered.filter(e => e.agentPlatform !== 'mcp')
  const mcpEntries = filtered.filter(e => e.agentPlatform === 'mcp')

  agentEntries.sort((a, b) => b.starsGained7d - a.starsGained7d)
  for (let i = 0; i < agentEntries.length; i++) agentEntries[i].rank = i + 1

  mcpEntries.sort((a, b) => b.starsGained7d - a.starsGained7d)
  for (let i = 0; i < mcpEntries.length; i++) mcpEntries[i].rank = i + 1

  const allEntries = [...agentEntries, ...mcpEntries]
  ...
  for (const entry of allEntries) {
    ...
    const derivedName = entry.repo.name || entry.repo.full_name?.split('/')[1] || entry.name.split('/')[1]
    const derivedFullName = entry.repo.full_name || entry.name
    const derivedUrl = entry.repo.html_url || `https://github.com/${derivedFullName}`
    const derivedHandle = entry.repo.owner?.login || entry.name.split('/')[0]
    ...
    projectRows.push({
      ...
      agent_platform: entry.agentPlatform || 'other',
  ...
  // --- 9. Upsert projects (skip rows missing required fields) ---
  const validRows = projectRows.filter(r => r.name && r.full_name)
  const skippedRows = projectRows.filter(r => !r.name || !r.full_name)
  if (skippedRows.length > 0) {
    console.warn(`Skipped ${skippedRows.length} rows missing name/full_name:`, skippedRows.map(r => r.id))
  }
  if (validRows.length > 0) {
    await supabaseUpsert('ranked_projects', validRows)
  }

  // --- 10. Insert snapshots (delete today's first to prevent duplicates) ---
  if (snapshotRows.length > 0) {
    const cfg = getSupabaseConfig()
    if (cfg) {
      await fetch(`${cfg.url}/rest/v1/ranked_snapshots?captured_at=eq.${today}`, {
        method: 'DELETE',
        headers: { apikey: cfg.key, Authorization: `Bearer ${cfg.key}` },
      })
    }
  ...
  for (const project of needReadme || []) {
    ...
    if (summary) {
      await supabasePatch('ranked_projects', project.id, { readme_summary: summary })
      readmeCount++
    }
  ...
  for (const project of needXHandle || []) {
    ...
    if (handle) {
      await supabasePatch('ranked_projects', project.id, { builder_x_handle: handle })
      xHandleCount++
    }
  ...
  for (const entry of allEntries) {
    const p = entry.agentPlatform || 'other'
    platformCounts[p] = (platformCounts[p] || 0) + 1
  }

  const summary = {
    fetched: totalFetched,
    filtered: allEntries.length,
    ranked: allEntries.length,
    newEntries: newCount,
    readmeSummaries: readmeCount,
    xHandles: xHandleCount,
    starred: starCount,
    draftsGenerated,
    flagged: allEntries.filter(e => e.reviewStatus !== 'approved').length,
    platformCounts,
  }

  const platformStr = Object.entries(platformCounts).map(([k, v]) => `${k}=${v}`).join(', ')
  console.log(`Pipeline complete: ${summary.fetched} repos fetched, ${summary.filtered} passed filter, ${summary.ranked} ranked, ${summary.newEntries} new entries. Upserted: ${platformStr}`)
