...
export async function runPipeline() {
  ...
  const { data: oldSnapshots } = await supabaseAdmin
    ...
    .select('project_id, stars_total')
    .eq('captured_at', sevenDaysAgo)
  ...
  const { data: yesterdaySnapshots } = await supabaseAdmin
    .from('ranked_snapshots')
  ...
  if (projectRows.length > 0) {
    ...
    if (upsertErr) {
      throw new Error(`Projects upsert failed: ${upsertErr.message}`)
    }
  }
  ...
  if (snapshotRows.length > 0) {
    const { error: snapErr } = await supabaseAdmin
      .from('ranked_snapshots')
      .upsert(snapshotRows, { onConflict: 'project_id,captured_at' })
    ...
    if (snapErr) {
      ...
      if (insertErr) {
        throw new Error(`Snapshots insert failed: ${insertErr.message}`)
      }
    }
  }
