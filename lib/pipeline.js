...
const PLATFORM_QUERIES = [
  ...
  // MCP Servers
  {
    ...
    signal: 'mcp-json',
    hintPlatform: 'mcp-server',
    extractRepo: (item) => item.repository,
  },
  {
    endpoint: '/search/repositories',
    q: 'topic:mcp-server pushed:>2024-06-01 stars:>2',
  ...
  {
    endpoint: '/search/repositories',
    q: 'topic:model-context-protocol pushed:>2024-06-01 stars:>2',
    signal: 'topic-mcp',
    hintPlatform: 'mcp-server',
    extractRepo: (item) => item,
  },
  {
    endpoint: '/search/repositories',
    q: '"mcp server" in:name,description topic:mcp pushed:>2024-06-01 stars:>3',
    signal: 'mcp-name-desc',
    hintPlatform: 'mcp-server',
    extractRepo: (item) => item,
  },
]
...
function isMcpServer(repo, fileTree, readmeText) {
  const files = (fileTree || []).map(f => f.path.toLowerCase())
  const topics = repo.topics || []
  const readme = (readmeText || '').toLowerCase()

  if (files.includes('mcp.json')) return true
  if (topics.includes('mcp-server') || topics.includes('model-context-protocol')) return true
  if (readme.includes('@modelcontextprotocol/sdk') ||
      readme.includes('npx -y @modelcontextprotocol')) return true
  if (topics.includes('mcp') && (
    repo.name?.toLowerCase().includes('mcp-server') ||
    repo.name?.toLowerCase().includes('-mcp-server') ||
    repo.name?.toLowerCase().startsWith('mcp-')
  )) return true
  return false
}
...
export async function runPipeline() {
  ...
  for (const entry of filtered) {
    ...
    if (signal === 'mcp-json') {
      entry.isMcpServer = true
      // Still need to detect agent_platform via file tree (could be claude-code, codex, etc.)
    }
    ...
    // Fetch README when needed for platform disambiguation or MCP detection
    const files = (fileTree || []).map(f => f.path.toLowerCase())
    const needsReadme = (
      (files.includes('agents.md') && !files.includes('claude.md') && !files.includes('openclaw.json')) ||
      (!entry.isMcpServer && (files.includes('mcp.json') || (repo.topics || []).includes('mcp')))
    )
    let readmeText = null
    if (needsReadme) {
    ...
    if (!entry.isMcpServer) {
      entry.isMcpServer = isMcpServer(repo, fileTree, readmeText)
    }
  }
  ...
  for (const entry of filtered) {
    ...
    projectRows.push({
      ...
      is_mcp_server: entry.isMcpServer || false,
  ...
  for (const entry of filtered) {
    ...
    if (entry.isMcpServer) mcpCount++
  }
  platformCounts['mcp-server'] = mcpCount
